...
excerpt: "ovos-busmon is a single web page that connects to your OVOS messagebus over WebSocket, with no install. Or run it as a small FastAPI service. Filter by type, session, or source; export JSONL; inject messages to reproduce bugs."
...
This post is for people who run or build OVOS and want to watch what it's doing internally. If that's not you, the short version: OVOS has a browser-based debugger for its internal event stream.

The OVOS messagebus carries every event the platform processes: utterances, intent matches, skill activations, TTS requests, session state. By default that stream goes to a terminal as scrolling text. Finding one message in a running system means writing a throwaway listener or grepping log files after the fact.

[`ovos-busmon`](https://github.com/OpenVoiceOS/ovos-busmon) puts the bus in your browser instead: filters, JSON inspection, capture export, and message injection when you need to force a bug to reproduce. Try it at [openvoiceos.github.io/ovos-busmon](https://openvoiceos.github.io/ovos-busmon/), with no install and no server.
...
With a messagebus running on the same machine, version 0.1.0 answers like this (HTTP Basic auth with the defaults; without credentials the same call returns 401):

```
$ curl -u ovos:ovos http://127.0.0.1:8005/api/status
{"version": "0.1.0", "buffered": 2, "buffer_capacity": 2000, "bus_host": "localhost", "bus_port": 8181}
$ curl -u ovos:ovos -X POST http://127.0.0.1:8005/api/send \
    -H 'Content-Type: application/json' -d '{"type":"speak","data":{"utterance":"hello from busmon"}}'
{"ok":true}
```
...
The service keeps a ring buffer (a fixed-size queue that overwrites its oldest entries once full) of recent messages, 2000 by default. A client that reconnects, or one you open a minute late, pages back through what it missed with `since_id` instead of starting blind.
...
- **Session ID, source and destination**, to isolate one conversation or one skill.
...
The hosted page reaches a device only when your browser can open a WebSocket to it, so a bus bound to localhost on another machine needs the service mode or an SSH tunnel. The ring buffer is memory only; a restart empties it. Bugs and questions go to the [issue tracker](https://github.com/OpenVoiceOS/ovos-busmon/issues).
