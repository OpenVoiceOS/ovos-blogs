...
With [`ovos-agentic-loop`](https://github.com/OpenVoiceOS/ovos-agentic-loop) your assistant can look things up, do multi-step tasks, and use tools before it answers you. Under the hood, each loop registers as an `opm.agents.chat` plugin, an OVOS building block for `ovos-persona`. What matters to you is simpler: ask something that needs a calculation, a search, or several steps, and the assistant goes and gets the pieces instead of guessing.
...
- **Plan-and-Execute** (`ovos-plan-execute-loop`): a planner LLM first writes a numbered list of 3 to 7 sub-tasks. In plain terms: it makes a to-do list before doing anything, then works through the list. Each step runs through its own mini-ReAct sub-loop, and a final call turns the collected results into the answer. The plan is visible up front, but this costs more LLM calls. It works best when a task needs more than about three tool calls, a point where plain ReAct starts to forget earlier observations.
...
- **Tree-of-Thoughts** (`ovos-tree-of-thoughts-loop`) is beam search over reasoning paths. In plain terms: it explores several possible next steps at once, scores them, and keeps only the most promising ones before going further. At each depth it generates several candidate steps, scores each with an evaluator LLM call, and keeps only the top `beam_width` branches for the next level. It can abandon a dead-end branch before committing to it, at the cost of the most LLM calls of any loop here. It suits problems with several competing strategies.
...
This section is for developers configuring a persona directly. Install the package (prerelease versions only, so ask for them):

```bash
pip install --pre ovos-agentic-loop
```

Version 0.2.3a2 registers these entry points, listed with `importlib.metadata` in a fresh environment:

```
opm.agents.chat 8 ['ovos-chain-of-thought-loop', 'ovos-critic-loop', 'ovos-native-toolcall-loop', 'ovos-plan-execute-loop', 'ovos-react-loop', 'ovos-reflexion-loop', 'ovos-self-ask-loop', 'ovos-tree-of-thoughts-loop']
opm.agents.toolbox 6 ['ovos-clock-tools', 'ovos-filesystem-tools', 'ovos-math-tools', 'ovos-shell-tools', 'ovos-skill-md-toolbox', 'ovos-web-search-tools']
```
...
One agentic plugin was the requirement. Eight loop architectures, six built-in toolboxes, and open MCP/UTCP tool discovery is what shipped.

## Limits

Every loop spends LLM calls; Tree-of-Thoughts and Plan-and-Execute spend the most, so a small local model gets slow. A loop is only as reliable as the brain's ability to follow its output format, which is why Native Tool-Call falls back to ReAct text. Bugs and questions go to the [issue tracker](https://github.com/OpenVoiceOS/ovos-agentic-loop/issues).
