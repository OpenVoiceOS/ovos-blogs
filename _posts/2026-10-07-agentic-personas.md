...
An OVOS persona used to be a single-shot thing: your words went to an LLM, the LLM answered, and that was the whole exchange. No looking things up, no multi-step reasoning, no calling a tool and coming back with a real answer.

The **"Consume UTCP/MCP/A2A → agentic plugin"** deliverable is complete, and it shipped more than the one plugin the MoU asked for: **seven** distinct reasoning-loop architectures and **five built-in toolboxes**, all as standard OPM plugins for `ovos-persona`.
...
- **ReAct** (`ovos-react-loop`) — interleaves reasoning and acting. Each iteration the LLM emits a *Thought → Action → Observation* triplet: think a step, call one tool, read the result, think again, until it emits `FINAL_ANSWER:`. One LLM call per iteration, but it cannot backtrack if an early tool call was wrong.
- **Plan-and-Execute** (`ovos-plan-execute-loop`) — a planner LLM first writes a numbered list of 3–7 sub-tasks; each step runs through its own mini-ReAct sub-loop; a final synthesize call turns the collected step outputs into the answer. The plan is auditable up front, at the cost of more LLM calls. Best when a task needs more than about three tool calls, where plain ReAct loses track of earlier observations.
...
- **Self-Ask** (`ovos-self-ask-loop`) — decomposes a multi-hop question into a chain of simpler follow-up questions, answering each (typically via one search tool) before asking the next. Its grammar is simpler than ReAct's JSON — a plain `query → result` — so smaller LLMs follow the format more reliably. Works with a single search tool, or with none at all as a pure decomposer.
- **Chain-of-Thought** (`ovos-chain-of-thought-loop`) — a single LLM call that reasons step by step before committing to a `FINAL ANSWER:`. No tools, no iteration — the lowest latency and cost of all seven. Good for arithmetic, logic puzzles, and multi-step instructions that need no external information; prone to hallucination on factual questions.
- **CRITIC** (`ovos-critic-loop`) — *draft → critique → revise*. The model drafts an answer, a second call flags the verifiable claims in it, each claim is checked with a tool, and only the incorrect facts are rewritten — the rest of the answer stays as written. Cheaper than Reflexion for factual corrections because it never re-runs the whole task. With no tools registered, it falls back to a plain draft.
- **Tree-of-Thoughts** (`ovos-tree-of-thoughts-loop`) — beam search over reasoning paths. At each depth it generates several candidate steps, scores each with an evaluator LLM call, and keeps only the top `beam_width` branches for the next level. It can explore and abandon a dead-end branch before committing, at the price of the most LLM calls of any loop here. Good for problems with several competing strategies.
...
A reasoning loop is only as good as the tools it can reach. `ovos-agentic-loop` ships five built-in toolboxes as `opm.agents.toolbox` plugins: `ovos-math-tools` (calculator, unit conversion, statistics, equation solving), `ovos-clock-tools`, `ovos-web-search-tools`, `ovos-filesystem-tools`, and `ovos-shell-tools`. The security defaults are conservative: the shell toolbox is disabled unless you set `allow_shell: true`, the filesystem toolbox can be pinned to a subtree via `root_path`, and the math toolbox parses expressions with an `ast` allowlist rather than calling `eval()`. A sixth toolbox, `ovos-skill-md-toolbox`, turns any installed `SKILL.md` file into an agent tool.
...
## Try it: config only, no code changes
