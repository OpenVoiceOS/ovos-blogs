...
Coding agents are everywhere, and for a lot of people, including people who would never call themselves programmers, "describe what you want and let an AI draft it" has become a useful way to make software do their bidding. For personal, private tinkering, that's not a threat. It's empowering.

We have our reservations about AI coding agents, and we're still working out how, and how much, to use them in our own development. But one thing is already clear: their most exciting place is in the hands of *users*. And a voice assistant you fully control is exactly the kind of software people should be able to bend to their own needs. So we're doing something concrete about it: **adding `AGENTS.md` and `SKILL.md` to the OpenVoiceOS repositories.** The rollout is partial: 23 of the 277 active repositories under the OpenVoiceOS organisation carry an `AGENTS.md` and 6 carry a `SKILL.md` (`gh search code filename:AGENTS.md --owner OpenVoiceOS`). This post describes the convention and where it is adopted.
...
As each repo gains its `AGENTS.md` and `SKILL.md`, working on it, by hand or with an assistant at your side, gets a little more approachable. That's the goal: not to build OVOS *with* AI, but to make OVOS something anyone can build *on*, whatever tools they bring. A repo you use that lacks the files is a reasonable issue to open on that repo.
