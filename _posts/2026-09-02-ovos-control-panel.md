...
The package has prerelease versions only, so `--pre` is needed. That starts it bound to `127.0.0.1:8500`, reachable only from the device
itself. Version 0.0.1a41 answers its own status route like this, with no token set:

```
$ curl http://127.0.0.1:8500/api/status
{"version":"0.0.1a41","host":"127.0.0.1","auth":false,"signed_in":true,"insecure":false,"warning":null,"has_token":false,"lang":"en-US"}
``` To reach it from another device on the network, bind it more openly
and set a token:
...
change is copying a backup back over the file, with no separate restore
mechanism to learn.

## The frontend leaned on AI

Backend and plugin code we can write and review ourselves without much
trouble. Visual design is not our strength. For the control panel's UI (layout, spacing, component choices, the responsive behavior across a phone
...
[`ovos-busmon`](https://github.com/OpenVoiceOS/ovos-busmon), which has
[its own post](https://blog.openvoiceos.org/posts/2026-09-09-debug-your-assistant-from-a-url). If you
...
directory. Bugs and questions go to the [issue tracker](https://github.com/OpenVoiceOS/ovos-control-panel/issues).
