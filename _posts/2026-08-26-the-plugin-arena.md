...
|---|---|---|
| `intent_zero_shot`, `intent_online`, `intent_offline`, `intent_keyword` | `benchmarks/intent_*.py` over [`intents-for-eval`](https://huggingface.co/datasets/OpenVoiceOS/intents-for-eval) and `massive-templates` | accuracy / macro-F1 / OOD false-positive rate / slot exact-match |
| `stt` | `benchmarks/stt_minds14.py` over MInDS-14, with further STT benchmarks beside it | word error rate |
| `wake_word` | `benchmarks/ww_hey_mycroft.py` over [`synthetic-wakewords`](https://huggingface.co/datasets/OpenVoiceOS/synthetic-wakewords) | detection error / false-accept / false-reject |
| `tts` | `benchmarks/tts_intents_prompts.py` | mean UTMOS with an STT round-trip intelligibility WER beside it |

The intent leagues are populated with published predictions. STT, TTS and wake-word fighters and datasets are registered and runnable, and their boards fill as prediction sweeps land; the wake-word league in particular is at smoke depth, a handful of fighters over one phrase.
...
A daily `assemble.yml` Action reads the HuggingFace prediction datasets and builds the battle pools. Each battle pairs two fighters' outputs on the *same* input and is shown to a voter anonymously: a blind comparison, where you judge the result without knowing which plugin produced it.
...
The vote log is the GitHub issue history: public, auditable and replayable from scratch at any time. There is no hidden database of votes to trust, only the issue tracker.
...
A brand-new arena would otherwise start cold, so the board is seeded from the objective benchmark scores. Before any human vote exists, the arena derives deterministic auto-battles from the benchmark metrics, one per sample where exactly one fighter got it right, and applies them at a quarter of the K-factor. Human votes then move ratings at full weight on top of that seed. The result is a board that already reflects measured quality on day one, and that human blind votes refine over time. The `tts` league is the exception: with no objective metric, it runs on human votes alone.

Because battles are blind and always between exactly two options, the vote dynamics are not skewed by which plugin is more famous, only by which output a listener preferred.
...
No servers, no databases, no accounts. Pages activates once the repo is public, which GitHub Pages on free plans requires, and from then on the arena maintains itself.
...
That turns plugin comparison from a one-off benchmark that goes stale into a community-owned resource that improves every time someone casts a vote.

Bugs, new fighters and dataset proposals go to the [issue tracker](https://github.com/OpenVoiceOS/ovos-plugin-arena/issues).
