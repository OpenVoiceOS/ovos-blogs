...
- `.voc` files are keyword lists: non-empty, with entries flagged if they look like whole sentences.
...
For the technical architecture (GitHub App token handling, the AST context extraction, the full validation rule set) see the [OVOS Localize whitepaper](https://github.com/OpenVoiceOS/ovos-localize/blob/dev/docs/whitepaper.md). Bugs and questions go to the [issue tracker](https://github.com/OpenVoiceOS/ovos-localize/issues).
