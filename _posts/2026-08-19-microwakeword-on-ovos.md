...
excerpt: "ovos-ww-plugin-microwakeword wraps the microWakeWord TFLite models built for ESPHome voice satellites, so the same wake words that run on an ESP32-S3 run unchanged on a full OVOS install and as an ordinary hotword plugin."
...
If your ESPHome voice satellite wakes on "Okay Nabu" or "Hey Jarvis", a full OVOS install wakes on the same model file. No retraining, no format conversion.

[microWakeWord](https://github.com/kahrendt/microWakeWord) is the TFLite-based wake word engine ESPHome built for its voice satellites. It runs streaming inference on microcontrollers, and the same models run on a full Linux box. The [ESPHome micro-wake-word-models](https://github.com/esphome/micro-wake-word-models) repository publishes a set of ready-to-use models, and any `.tflite` file that follows the microWakeWord input convention works out of the box.
...
Install the plugin. The package has prerelease versions only, so ask for them:

```bash
pip install --pre ovos-ww-plugin-microwakeword
```
...
| `refractory_frames` | `40` | Frames (about 400 ms) ignored after a trigger, so one spoken phrase does not fire twice. |

Loading a model by name downloads it once and caches it. This is what version 0.0.1a3 prints when the plugin is created and then fed one second of silence:

```python
import numpy as np
from ovos_ww_plugin_microwakeword import MicroWakeWordPlugin

p = MicroWakeWordPlugin(key_phrase="okay nabu",
                        config={"model_name": "okay_nabu", "model_version": 1})
for _ in range(10):
...
```
INFO - Loading microWakeWord model: /home/user/.local/share/microwakeword/okay_nabu.tflite
...
microWakeWord models are trained on human speech and are picky about it. In the plugin's own end-to-end tests, synthesized TTS audio of the wake phrase does not reliably trigger detection. The positive-path test soft-skips for that reason, while the negative case (a phrase that should never fire) is a hard assertion. Validate a wake word by speaking it, not by piping a TTS clip through the recognizer. If a model seems unresponsive on synthetic audio, that is expected, not a broken install.

## When the Phrase You Want Isn't in the Catalogue

The ESPHome catalogue is convenient but finite. If you want a wake phrase nobody has published (a product name, a personal assistant name, a language the catalogue does not cover) you have to train one. [WakeForge](https://github.com/TigreGotico/wakeforge) is a training pipeline for your own OVOS wake word models, and [`ovos-ww-plugin-wakeforge`](https://github.com/OpenVoiceOS/ovos-ww-plugin-wakeforge) runs them. microWakeWord gives you an existing library of phrases. WakeForge covers what that library is missing.

Bugs and questions go to the [issue tracker](https://github.com/OpenVoiceOS/ovos-ww-plugin-microwakeword/issues).
