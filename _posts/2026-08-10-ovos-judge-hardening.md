...
Separately, a functional bug turned up that had nothing to do with security and everything to do with test discipline — and, honestly, it was one *we* introduced during this very pass. A change meant to "modernise" the persona-drive call rewrote `Persona.chat()` to match a newer `ovos-persona` signature that happened to be installed in a local development environment — but not the version ovos-judge actually ships against (`ovos-persona 0.7.1`, whose `Persona.chat` takes `(messages, lang, units)` and reads the messages as plain dicts). Against the real dependency the call broke: the persona answered nothing, and the judge dutifully marked every single generated utterance as a failure. Every unit test mocked `persona.chat()` with a permissive signature, so the mismatch never surfaced — the suite stayed green while the core "drive a persona" path was dead. We reverted the call to what the shipped dependency expects and replaced the test with one that mimics the real 0.7.1 contract instead of a mock that accepts anything. The lesson landed twice: verify against the version you actually resolve, not the one your dev box happens to have.
